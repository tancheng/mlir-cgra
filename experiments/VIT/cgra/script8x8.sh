...
# perform tiling
soda-opt --linalg-tiling-for-cgra=cgra-configs=8,8,4096,4096,4096,4096 04-finalized.mlir > 05-tiled-8.mlir

# locating target operations that will be offloaded onto CGRA
 soda-opt --convert-linalg-matmul-to-cgra --convert-linalg-batch_matmul-to-cgra --convert-pattern-to-cgra=target-patterns="add-add","div-sub","add","mul-add","trunc-div-add" --convert-linalg-generic-to-cgra 05-tiled-8.mlir > 06-locating-8.mlir
...
clang++-12 main.cpp 11-model-8.o 12-accel-8.o ../../../../llvm-project/build/lib/libmlir_c_runner_utils.so CustomizedRuntime-8.cpp -I../../../sim/ ../../../sim/*.cpp -o simulate8

./simulate8 8 false false
