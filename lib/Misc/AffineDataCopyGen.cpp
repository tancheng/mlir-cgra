//===- AffineDataCopyGen.cpp - Explicit memref copying pass ------*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is a copy of AffineDataCopyGeneration.cpp with added bool option
// to disable affine.DMA ops generation.
// TODO: Push these changes to llvm-project
//
...
struct AffineDataCopyGen
    : public AffineDataCopyGenerationBase<AffineDataCopyGen> {
  AffineDataCopyGen() = default;
  explicit AffineDataCopyGen(unsigned slowMemorySpace, unsigned fastMemorySpace,
                             unsigned tagMemorySpace, int minDmaTransferSize,
                             uint64_t fastMemCapacityBytes, bool generateDma) {
...
mlir::soda::createAffineDataCopyGenPass(
  ...
  return std::make_unique<AffineDataCopyGen>(slowMemorySpace, fastMemorySpace,
                                             ...
                                             fastMemCapacityBytes, generateDma);
}
