//===- AffineLoopTiling.cpp --- Loop tiling pass ------------------------*-===//
//
...
void AffineLoopTiling::getTileSizes(ArrayRef<AffineForOp> band,
                                    SmallVectorImpl<unsigned> *tileSizes) {
  if (band.empty())
    return;
  ...
  if (!fp) {
    ...
    std::fill(tileSizes->begin(), tileSizes->end(),
              AffineLoopTiling::kDefaultTileSize);
...
void AffineLoopTiling::runOnOperation() {
...
constexpr unsigned AffineLoopTiling::kDefaultTileSize;
