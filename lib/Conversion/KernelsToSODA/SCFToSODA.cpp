...
// #include "mlir/Dialect/SODA/ParallelLoopMapper.h"
...
static Value getDim3Value(const soda::KernelDim3 &dim3, unsigned pos) {
  switch (pos) {
  case 0:
    return dim3.x;
  case 1:
...
// mapping a loop nest of depth "numLoops" rooted at "forOp" to a SODA kernel.
...
AffineLoopToSodaConverter::collectBounds(AffineForOp forOp, unsigned numLoops) {
...
// Replace the rooted at "rootForOp" with a SODA launch operation.  This expects
...
void AffineLoopToSodaConverter::createLaunch(AffineForOp rootForOp,
  ...
  // launch op.
  auto launchOp = builder.create<soda::LaunchOp>(
  ...
  // soda terminator and move the operations from the loop body block to the soda
  ...
  builder.create<soda::TerminatorOp>(terminatorLoc, llvm::None);
  ...
  // may iterate from LB with step S whereas SODA thread/block ids always iterate
  // from 0 to N with step 1.  Therefore, loop induction variables are replaced
  // with (soda-thread/block-id * S) + LB.
...
// Generic loop to SODA kernel conversion function.
...
LogicalResult mlir::convertAffineLoopNestToSODALaunch(AffineForOp forOp,
                                                     unsigned numBlockDims,
                                                     unsigned numThreadDims) {
  return ::convertAffineLoopNestToSODALaunch(forOp, numBlockDims, numThreadDims);
}

namespace {
struct ParallelToSodaLaunchLowering : public OpRewritePattern<ParallelOp> {
...
static bool isMappedToProcessor(soda::Processor processor) {
  return processor != soda::Processor::Sequential;
}

static unsigned getLaunchOpArgumentNum(soda::Processor processor) {
  switch (processor) {
  case soda::Processor::BlockX:
    return 0;
  case soda::Processor::BlockY:
    return 1;
  case soda::Processor::BlockZ:
    return 2;
  case soda::Processor::ThreadX:
    return 3;
  case soda::Processor::ThreadY:
    return 4;
  case soda::Processor::ThreadZ:
    return 5;
  default:;
  }
  llvm_unreachable(
...
///   `soda.launch operation` accordingly.
/// - if the upper bound is imprecise, insert a conditional in the `soda.launch`
...
/// inserted, a sentinel (the `soda.launch` operation) is inserted into the
/// worklist. This signals the processor of the worklist to pop the rewriter
/// one scope-level up.
static LogicalResult processParallelLoop(
    ParallelOp parallelOp, soda::LaunchOp launchOp,
    BlockAndValueMapping &cloningMap, SmallVectorImpl<Operation *> &worklist,
    DenseMap<soda::Processor, Value> &bounds, PatternRewriter &rewriter) {
  // TODO: Verify that this is a valid SODA mapping.
  // processor ids: 0-2 block [x/y/z], 3-5 -> thread [x/y/z], 6-> sequential
  ArrayAttr mapping =
      parallelOp.getAttrOfType<ArrayAttr>(soda::getMappingAttrName());
  ...
  for (auto config : llvm::zip(mapping, parallelOp.getInductionVars(),
    ...
    auto annotation = mappingAttribute.dyn_cast<soda::ParallelLoopDimMapping>();
    if (!annotation)
      return parallelOp.emitOpError()
             << "expected mapping attribute for lowering to SODA";
    Value newIndex;
    soda::Processor processor = soda::getProcessor(annotation);
  ...
  // later stage, such as extension data for SODA kernel dispatch
  for (const auto &namedAttr : parallelOp.getAttrs()) {
    if (namedAttr.first == soda::getMappingAttrName() ||
...
/// Lower a `scf.parallel` operation into a corresponding `soda.launch`
/// operation.
///
...
/// operations.
LogicalResult
ParallelToSodaLaunchLowering::matchAndRewrite(ParallelOp parallelOp,
  ...
  soda::LaunchOp launchOp = rewriter.create<soda::LaunchOp>(
      parallelOp.getLoc(), constantOne, constantOne, constantOne, constantOne,
      constantOne, constantOne);
  rewriter.setInsertionPointToEnd(&launchOp.body().front());
  rewriter.create<soda::TerminatorOp>(loc);
  rewriter.setInsertionPointToStart(&launchOp.body().front());

  BlockAndValueMapping cloningMap;
  llvm::DenseMap<soda::Processor, Value> launchBounds;
...
void mlir::populateParallelLoopToSODAPatterns(OwningRewritePatternList &patterns,
                                             MLIRContext *ctx) {
  patterns.insert<ParallelToSodaLaunchLowering>(ctx);
}
